...
public class MediumDriverAdminController {
    ...
    MagpiePage<MediumDriverInfo> getAllDriverInfo(@ApiParam(value = "渠道 ID", hidden = true)
        ...
        return mediumDriverInfoService.findAllByChannelIdDesc(channelId, page, size);
    }
    ...
    MagpiePage<MediumDriverOrder> getAllDriverOrder(@ApiParam(value = "渠道 ID", hidden = true)
                                              ...
                                              @RequestParam(name = "page", required = false, defaultValue = "0") Integer page,
                                                    @ApiParam(value = "页大小")
                                              @RequestParam(name = "size", required = false, defaultValue = "20") Integer size) {
        return mediumDriverOrderService.findAllByChannelIdDesc(channelId, page, size);
    }
    ...
    MagpiePage<MediumDriverOrder> getAllDriverOrderByUserId(@ApiParam(value = "渠道 ID", hidden = true)
                                                      ...
                                                      @RequestParam(name = "page", required = false, defaultValue = "0") Integer page,
                                                            @ApiParam(value = "页大小")
                                                      @RequestParam(name = "size", required = false, defaultValue = "20") Integer size) {
        return mediumDriverOrderService.findAllByChannelIdAndUserId(channelId, userId, page, size);
    }
}
